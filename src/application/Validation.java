...
public class Validation {
	...
	public static Boolean validateRegister(String fullName, String email, String pass, String cfr_pass, String address, String DOB, String gender, Boolean tnc) {
		...
		if(email.isEmpty()
			...
			|| email.chars().filter(ch -> ch == '@').count() > 1
			|| email.substring(email.indexOf('@')+1, email.length()).chars().filter(ch -> ch == '.').count() < 1
			) {
