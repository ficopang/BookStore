...
public class DatabaseConnection {
    ...
    public static void getAllCarts(TableView<Cart> tableView, int userId) {
        try {
	        ...
	        String sql = "SELECT * FROM `cart` as c LEFT JOIN book as bk ON c.BookID = bk.BookID WHERE c.UserID = " + userId;
	        rs = stmt.executeQuery(sql);
	        while(rs.next()){
    ...
    public static void getAllBooks(TableView<Book> tableView) {
        try {
	        ...
	        rs = stmt.executeQuery(sql);
	        while(rs.next()){
	        	tableView.getItems().add(new Book(
		        			rs.getInt("BookID"),
		        			rs.getString("BookName"),
		        			rs.getString("BookAuthor"),
		        			rs.getInt("GenreID"),
    ...
    public static void getAllGenres(TableView<Genre> tableView) {
        try {
	        ...
	        rs = stmt.executeQuery(sql);
	        while(rs.next()){
	        	tableView.getItems().add(new Genre(
		        			rs.getInt("GenreId"),
		        			rs.getString("GenreName")
	        			));
	        }
	...
	public static Boolean updateGenre(int id, String name) {
        try {
        	String sql = "UPDATE genre set GenreName = '%s' WHERE GenreId = %d";
	...
	public static Boolean deleteGenre(int id) {
        try {
        	String sql = "DELETE FROM genre WHERE GenreId = %d";
	        sql = String.format(sql, id);
	        int count = stmt.executeUpdate(sql);
	        if(count > 0) return true;
        } catch (Exception e) {
	...
	public static Boolean registerUser(String fullName, String email, String password, String address, String dob, String gender) {
	    try {  
	...
	public static Boolean loginUser(String email, String password) {
        ...
        try {
            ...
            e.printStackTrace();
			AlertWindow.show(AlertType.ERROR, "Book already in cart!");
        }
